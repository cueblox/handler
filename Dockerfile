...
WORKDIR ${GOPATH}/src/github.com/cueblox/handler
...
# Run quality control
#RUN go test -test.v -test.race -test.covermode=atomic ./cmd/handler
#RUN golangci-lint run ./cmd/handler

# Compile imaginary
RUN go build -a \
    -o ${GOPATH}/bin/handler \
    -ldflags="-s -w -h -X main.Version=${IMAGINARY_VERSION}" \
    github.com/cueblox/handler
...
COPY --from=builder /go/bin/handler /usr/local/bin/handler
...
EXPOSE ${PORT}
CMD ["-enable-url-source", "-cors", "-gzip", "-mount", "/data/static"]
